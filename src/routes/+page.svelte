<script>
    import { AUTH_CONFIG } from "$lib/common/constants";
	import { randomString } from "$lib/common/functions";
    import Loader from "$lib/components/Loader.svelte";
    import { parseJWT } from "$lib/common/functions";
    import { changeAuthState } from "$lib/stores/auth.store";
  
    const handleLogin = () => {
        const nonce = encodeURIComponent(randomString());
...
<main class="lead-content">
    {#if urlSearchParams.get('access_token')}
        <div class="flex">
    ...
    {:else }
        <section id="sign-in-section">
            <h1 class="lead-head stroked-text">$core$cribe</h1>
            <p class="lead-text">sign in to continue</p>

            <button class="gsi-material-button" on:click={handleLogin}>
...
<style>
    ...
    #sign-in-section .lead-head {
        font-size: var(--size-xl);
        font-weight: 800;
    }
    #sign-in-section .lead-text {
